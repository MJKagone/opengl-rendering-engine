...
Run a specific scene via CLI, e.g. `./build/main --scene scenes/boat.json` (verify exact flag in `src/main.cpp`).
...
- **`scenes/*.json`** define skybox/IBL environment, lights, and entities (model path + transform). Colors are specified 0–255 RGB, converted sRGB→linear by `Scene.hpp`/`main.cpp`.
